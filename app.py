...
if not firebase_admin._apps:
    ...
    firebase_creds = os.getenv('FB_CREDS')

    # Parse the JSON string into a dictionary
    creds_dict = json.loads(firebase_creds)

    # Initialize Firebase app with parsed credentials
...
def sign_in():
    ...
    if submit_button:
        try:
            ...
            sign_in_url = f"https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key={FB_KEY}"
            ...
            # Make a POST request to Firebase Auth
            response = requests.post(sign_in_url, data=json.dumps(payload), headers={"Content-Type": "application/json"})
            response_data = response.json()

            if response.status_code == 200:
                # Extract the idToken and localId (equivalent to uid)
                id_token = response_data.get('idToken')
                user_id = response_data.get('localId')  # This is the uid
                
                if id_token and user_id:
                    # Store session data
                    st.session_state.user = response_data
                    st.session_state.authentication_status = True
                    st.success(f"Signed in successfully! Hello User!")
                    
                    # Call additional functions after successful login
                    load_canvas_data()
                    st.experimental_rerun()
                else:
                    st.error("Failed to retrieve authentication token or user ID.")

            else:
                # Display detailed Firebase error message
                error_message = response_data.get('error', {}).get('message', 'Unknown error occurred')
                st.error(f"Error during sign in: {error_message}")
...
def save_canvas_data(canvas_data):
    if st.session_state.user:
        user_id = st.session_state.user['localId'] # Use 'uid' with firebase-admin
        user_ref = realtime_db.reference(f"users/{user_id}/canvas")
        user_ref.set(canvas_data)
...
def store_document(text, metadata):
    ...
    doc_ref = realtime_db.reference(f'rag_documents/{doc_id}')
    doc_ref.set(doc_data)
    return doc_id

def retrieve_similar_documents(query_vector, top_k=3):
    all_docs_ref = realtime_db.reference('rag_documents')
    all_docs = all_docs_ref.get()
    if not all_docs:
        return []
...
def ipdr_analysis():
    ...
    if uploaded_file is not None:
        ...
        with col1:
            with st.container():
                options = {
                    "Frequent IP Source": ('Destination.IP', 'Times Called'),
                    "Frequent IP Destinations": ('Source.IP', 'Times Called'),
                    "High Data Usage Sessions": ('Flow.Bytes.s', 'Data Transferred'),
                    "Long Duration Calls": ('Flow.Duration', 'Duration'),
        ...
        with col2:
            with st.container():
                ...
                for _, row in df.iterrows():
                    caller = row['Source.IP']
                    called = row['Destination.IP']
        ...
        with col1:
            with st.container():
                ...
                if "Flow.Duration" in df.columns:
                    fig = px.histogram(df, x="Flow.Duration", title="Call Duration Distribution", nbins=50)
                    fig.update_layout(height=300)
                    st.plotly_chart(fig)
                else:
                    st.warning("Flow.Duration column not found in the dataset.")
        
        with col2:
            with st.container():
                st.markdown("### Data Usage Analysis")
                if "Flow.Bytes.s" in df.columns:
                    fig = px.histogram(df, x="Flow.Bytes.s", title="Data Usage Distribution", nbins=50)
                    fig.update_layout(height=300)
                    st.plotly_chart(fig)
                    
                    st.write(f"Total Data Transferred: {df['Flow.Bytes.s'].sum()} bytes")
                    st.write(f"Average Data per Session: {df['Flow.Bytes.s'].mean():.2f} bytes")
                else:
                    st.warning("Flow.Bytes.s column not found in the dataset.")
        
        with col3:
            with st.container():
                st.markdown("### IP Address Analysis")
                if "Source.Port" in df.columns and "Destination.Port" in df.columns:
                    internal_ips = df['Source.Port'].nunique()
                    external_ips = df['Destination.Port'].nunique()
                    ...
                    ip_counts = df['Source.Port'].value_counts().head(10)
                    fig = px.bar(x=ip_counts.index, y=ip_counts.values, title="Top 10 Source.Portes")
                    fig.update_layout(height=300)
                    st.plotly_chart(fig)
                else:
                    st.warning("Source.Port or Destination.Port columns not found in the dataset.")
        ...
        with col2:
            with st.container():
                ...
                if "ProtocolName" in df.columns:
                    service_usage = df['ProtocolName'].value_counts()
                    fig = px.pie(values=service_usage.values, names=service_usage.index, title='Protocols Distribution')
                    fig.update_layout(height=300)
                    st.plotly_chart(fig)
                else:
        ...
        Total Data Transferred: {df['Flow.Bytes.s'].sum() if 'Flow.Bytes.s' in df.columns else 'N/A'} bytes
        Unique Callers: {df['Source.IP'].nunique() if 'Source.IP' in df.columns else 'N/A'}
        Unique Called Numbers: {df['Destination.IP'].nunique() if 'Destination.IP' in df.columns else 'N/A'}
        Most Common Service: {df['Service Used'].mode().values[0] if 'Service Used' in df.columns else 'N/A'}
        Most Common ProtocolName: {df['ProtocolName'].mode().values[0] if 'ProtocolName Used' in df.columns else 'N/A'}
...
def generate_pdf_report(df, insights):
    ...
    story.append(Paragraph(f"Total Data Transferred: {df['Flow.Bytes.s'].sum()} bytes", styles['Normal']))
    story.append(Spacer(1, 12))
...
def generate_key_statistics(df):
    return {
        ...
        "total_data_transferred": df['Flow.Bytes.s'].sum(),
        "unique_callers": df['Source.IP'].nunique(),
        "unique_called_numbers": df['Destination.IP'].nunique(),
        "most_common_service": df['Service Used'].mode().values[0],
        "most_common_ProtocolName": df['ProtocolName'].mode().values[0],
...
def main():
    ...
    st.title("🤖CybSec🤖")
    st.write("One of a kind LLM-powered Investigation tool for cybersecurity analysts")
    ...
    if st.session_state.authentication_status != True:
        ...
        tab1, tab2 = st.tabs(["Analysis Tools","Interactive Board"])
        
        with tab2:
            col1, col2 = st.columns([3, 1])
            
            with col1:
                st.header("Interactive Board")
        ...
        with tab1:
            ...
            with tool_tabs[2]:  
                ...
                # Removed wallet connection section
                        ...
                        if st.button("Upload to IPFS"):
                            with st.spinner("Uploading file to IPFS..."):
                                ...
                                if ipfs_hash:
                                    st.success(f"File uploaded to IPFS. Hash: {ipfs_hash}")
                                    user_id = st.session_state.user['localId'] # Use 'uid' with firebase-admin
                                    ipfs_files_ref = realtime_db.reference(f"users/{user_id}/ipfs_files")
                                    ipfs_files_ref.push({"name": uploaded_file.name, "hash": ipfs_hash})
                
                # View Files Tab
                with ipfs_tabs[1]:
                    st.subheader("View IPFS Files")
                    user_id = st.session_state.user['localId']# Use 'uid' with firebase-admin
                    user_files_ref = realtime_db.reference(f"users/{user_id}/ipfs_files")
                    user_files = user_files_ref.get()
            ...
            st.markdown("Made by Team RAG2riches", unsafe_allow_html=True)
